...
const MoviesEdit = () => {
  ...
  const getValueName = (param: any) => {
    let actors = "";
    let emptyArr: string[] = [];
    param.forEach((value: string) => {
      options.forEach((options: any) => {
        if (value === options.id) {
          actors = options.value;
        }
      });
      emptyArr.push(actors);
    });
    return emptyArr;
  };
              ...
              <Input.Group compact>
                <Space direction="vertical" style={{ width: "100%" }}>
                  <Select
                    ...
                    style={{ width: "100%" }}
                    options={options}
                    defaultValue={() => getValueName(location.state.actorsId)}
                    onChange={(value: any) => {
                      let array: string[] = [];
                      value.forEach((data: string) => {
                        const id: any = getValueId(data);
                        array.push(id);
                      });
                      setValue(array);
                    }}
